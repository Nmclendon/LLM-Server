...
<div
	...
	class="h-screen max-h-[100dvh] min-h-screen select-none {$showSidebar
		? 'md:relative w-[260px]'
		: '-translate-x-[260px] w-[0px]'} bg-sidebar text-gray-900 pastel:bg-snb-700 pastel:text-gray-900 dark:bg-maroon-700 dark:text-gold-700 text-sm transition fixed z-50 top-0 left-0 rounded-r-2xl
        "
...
>
	<div
		class="py-2.5 my-auto flex flex-col justify-between h-screen max-h-[100dvh] w-[260px] z-50 {$showSidebar
			...
			: 'invisible'}"
	>
		<div class="px-2.5 flex justify-between space-x-1 text-gray-600 dark:text-gray-950">
			<a
				id="sidebar-new-chat-button"
				class="flex flex-1 justify-between rounded-xl px-2 py-2 bg-sidebar-accent dark:bg-gold-700 hover:bg-sidebar-accent-hover dark:hover:bg-gold-700 transition"
				href="/"
				draggable="false"
				on:click={async () => {
					selectedChatId = null;
			...
			>
				...
				<div class=" self-center font-medium text-sm text-sidebar-accent-text-color dark:text-black font-primary">
					{$i18n.t('New Chat')}
				</div>
			...
			<button
				class=" cursor-pointer px-2 py-2 flex rounded-xl hover:bg-gray-100 pastel:hover:bg-sny-700 dark:hover:bg-maroon-850 transition"
				on:click={() => {
					showSidebar.set(!$showSidebar);
				}}
			>
				<div class=" m-auto self-center dark:text-gold-700">
					<svg
						xmlns="http://www.w3.org/2000/svg"
						fill="none"
						viewBox="0 0 24 24"
		...
		{#if $user?.role === 'admin'}
			<div class="px-2.5 flex justify-center text-gray-800 dark:text-gold-700">
				<a
					class="flex-grow flex space-x-3 rounded-xl px-2.5 py-2 hover:bg-sidebar-hover pastel:hover:bg-sny-700 dark:hover:bg-maroon-850 transition"
					href="/workspace"
					on:click={() => {
						selectedChatId = null;
						chatId.set('');
		...
		<div class="relative flex flex-col flex-1 overflow-y-auto">
			...
			{#if $tags.filter((t) => t.name !== 'pinned').length > 0}
				<div class="px-2.5 mb-2 flex gap-1 flex-wrap">
					<button
						class="px-2.5 text-xs font-medium bg-gray-50 pastel:bg-sny-700 pastel:hover:bg-sny-700 dark:bg-gray-900 dark:hover:bg-gray-800 transition rounded-full"
						on:click={async () => {
							await chats.set(await getChatList(localStorage.token));
						}}
					>
			...
			<div class="pl-2 my-2 flex-1 flex flex-col space-y-1 overflow-y-auto scrollbar-hidden">
				{#each filteredChatList as chat, idx}
					{#if idx === 0 || (idx > 0 && chat.time_range !== filteredChatList[idx - 1].time_range)}
						<div
							class="w-full pl-2.5 text-xs text-gray-500 dark:text-gold-700 font-medium {idx === 0
								? ''
								: 'pt-5'} pb-0.5"
						>
		...
		<div class="px-2.5">
			...
			<div class="flex flex-col font-primary">
				{#if $user !== undefined}
					...
					>
						<button
							class=" flex rounded-xl py-3 px-3.5 w-full hover:bg-gray-100 dark:hover:bg-maroon-850 transition"
							on:click={() => {
